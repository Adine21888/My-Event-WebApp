...
const TeamSearchPostList = () => {
  ...
  const fetchTeamSearchPosts = async () => {
    try {
      const response = await axios.get('http://localhost:5555/team-search-posts');
  ...
  const handleInterest = async (postId) => {
    try {
      const response = await axios.post(`http://localhost:5555/team-search-posts/${postId}/interested`, {}, {
